...
//! 決定論的 socsim コアからは [`EconClient`] 型エイリアス経由でのみ触れる．
//!
...
//! 再実行を擬似決定論化する．キャッシュは `socsim-llm` の features=["live"]
//! 経由で有効化される (Ollama + OpenAI 両バックエンド)．
//!
//! 設計書 §4.2 は当初 `reqwest` + 手書き `llm.rs` を挙げていたが，本スイートは
//! `socsim-llm` クレット (issue #21/#26) に標準化したため `reqwest` は使わず，
//! HTTP は socsim-llm が所有する．
//!
//! テストでは `socsim-llm::mock::ScriptedClient` を `Box<dyn LlmClient>` として
//! 同じ [`EconClient`] に流し込める．`socsim-llm` が `Box<dyn LlmClient>` に対する
...
pub type EconClient = CachingClient<Box<dyn LlmClient>>;
...
///   `gpt-3.5-turbo-0613`)．未設定なら空キーのフォールバックを置く (Ollama が
///   成功すれば呼ばれない; 両方失敗時のみ設定エラーになる)．
/// - キャッシュ: `settings.cache_path` があればその JSON ファイル，なければ
///   in-memory．
pub fn build_live_client(settings: &LlmSettings) -> Result<EconClient, LlmError> {
...
/// [`EconClient`] を作る (主にテスト用)．
pub fn wrap_client<C: LlmClient + 'static>(backend: C, cache: PromptCache) -> EconClient {
